# Detailed Agent Reference — Paseo

Supplement to [`../AGENTS.md`](../AGENTS.md). The canonical prompt contains the
load-bearing rules; this file retains the full routing tables and worked examples.
...
| [docs/design.md](design.md)                               | Theme tokens — colors, fonts, spacing, radii, icons                                                                            |
...
| [docs/android.md](android.md)                             | App variants, local/cloud builds, EAS workflows                                                                                |
...
See [docs/development.md](development.md) for full setup, build sync requirements, and debugging.
...
- **The protocol stays backward-compatible. Features don't have to.** Two separate contracts:
  ...
  - **Feature contract (per-feature):** a new feature may require a new daemon capability. The client detects whether the capability is present and either runs the feature or shows "Update the host to use this." That's it.
    ...
    - **New RPCs use dotted namespaces with direction suffixes.** Follow [docs/rpc-namespacing.md](rpc-namespacing.md): `domain.provider.operation.request` pairs with `domain.provider.operation.response`. Existing flat RPC names will migrate over time; don't add new ones.
